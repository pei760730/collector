...
訊息文字
  ...
  → extractVideoId  帶平台前綴的唯一 ID(tiktok_/ig_/yt_/xhs_/threads_;抓不到 unknown_<ts>)
...
| 無網址 / 格式錯誤 | 回格式錯誤提示 + 範例 |
| `/stats` | 總筆數 + 各平台 + 本週/本月新增 + 最近 5 筆 |
| `/pick R#### [R#### …]` | 在 voc「參考池」按編碼把「挑」欄打勾,交 `voc pick` 搬進待拍(bot 只打勾不搬表) |

> `/stats` 是「邊做邊修」的預設版,handler 介面已留好。`/move` 已隨暫存區瘦身(砍 STATUS 欄)退役。
...
| `DEDUPE_PERIOD_DAYS` | 去重時間窗,也是 prune 清理窗(預設 14) |
...
- voc 精簡後只按表頭名讀 `PLATFORM / CLEAN_URL / DATE` 3 欄(改這幾欄名要兩 repo 一起改);其餘細節留在暫存區當底料。
