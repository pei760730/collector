...
> **of 引擎 target 例外(`of` / `ofgay`)**:走 `src/engines/of/` vendored 獨立引擎(原 feed-collector,2026-07-15 併入,舊 repo archive)——暫存區英文 5 欄、STATUS 狀態流、tt_/dy_ 前綴,與 voc/tbvoc 刻意不同 scope(owner 拍板別硬拉 parity),**不在 TargetSpec 內**;`src/drain.ts` 依 `targets.ts` 的 `OF_ENGINE_TARGETS`(`isOfTarget`)委派,本檔以下各層描述**只適用 voc/tbvoc 殼**,of 引擎的契約/不變式見 `src/engines/of/` 內檔頭註解與 `tests/of/`。**`ofgay`(2026-08-08 加入)= 同一支引擎跑 gay 市場**:換 bot、換表(`OFGAY_` 前綴 secrets),引擎程式碼零差異;要再開市場照這個形狀加(`OF_ENGINE_TARGETS` 一個名字 + collect.yml 一筆 matrix),別 fork 引擎。上游 tbvoc 契約(原 clip-collector)同日併入:`src/targets.ts` + `contracts/tbvoc/` + `tests/contractTbvoc.test.ts`,SSoT 在 TeaBus-VOC repo。
...
| 去重 key 演算法(連結→key) | `src/pipeline/index.ts`:`dedupKey` = core `groupKey` re-export(平台:影片id 優先,抽不到退連結路徑;對齊 voc `cli._dedup_key`,原 `sync._dedup_key`、sync.py 砍除後邏輯搬到 cli) |
...
- **契約欄位 = voc `schema.REFS` 4 欄(改名要兩 repo 一起)**。bot append 依**實際表頭具名解析**(`resolveHeaderIndexes`+`placeRow`,容忍表頭重排/前置多欄,必要欄缺失才 fail-fast);由 `tests/contract.test.ts` + `tests/headerMap.test.ts` 守(改欄名 → CI 紅):
...
- **去重(寫入前,bot 端負責)**:`src/pipeline/index.ts` 的 `dedupKey` = core `groupKey` re-export,對齊 voc `cli._dedup_key`(原 `sync._dedup_key`,sync.py 砍除後邏輯搬到 cli)—— 優先「平台:影片id」(讓 youtu.be/watch?v=/shorts 收斂),抽不到才退連結路徑(砍 query/fragment + 去尾斜線 + lower)。`collect` 寫入前讀現有「連結」欄、候選與既有列同支推 key 比對,重複跳過。**全表比對、無時間窗**(參考池永久池,不 prune)。
