...
從 private repo `pei760730/voc` 的 `contracts/` 複製過來,給 `tests/contract.test.ts` 跑 conformance。
...
```bash
cp D:/dev/voc/contracts/{schema.json,dedup_vectors.json} contracts/voc/
```
...
voc CI 改 `contracts/` 時自動對三個 collector 開「vendor 更新 PR」。卡在跨 repo token(voc private),待解(設計 §4 fan-out)。在那之前靠這份 README 手動同步 + conformance 紅燈擋漂移。
